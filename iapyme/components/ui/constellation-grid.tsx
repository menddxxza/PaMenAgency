...
interface ConstellationGridProps {
  className?: string;
  /** Título y subtítulo superpuestos. Por defecto, igual que el demo original. */
  showOverlay?: boolean;
  /** Ignora prefers-color-scheme y fuerza la paleta oscura (para usarlo sobre
   * fondos oscuros fijos del sitio, que no dependen del tema del sistema). */
  forceDark?: boolean;
}
...
export default function ConstellationGrid({
  ...
  return (
    <div className={cn('relative h-screen w-full select-none overflow-hidden bg-slate-950', className)}>
      {/* h-full/w-full explícitos: un <canvas> es un elemento reemplazado, así
          que "absolute inset-0" solo no lo estira — sin tamaño CSS propio
          usa su tamaño intrínseco por defecto (300x150), y el código de
          abajo mide ESE tamaño roto al arrancar en vez del contenedor real. */}
